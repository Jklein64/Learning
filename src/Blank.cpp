...
struct BlankModule : Module {
  ...
  // To make this module more than a blanking plate, configure inputs and
  // outputs in the Module constructor
  //
  // BlankModule()
  //{
  //    config(Params::NUM_PARAMS, Inputs::NUM_INPUTS, Outputs::NUM_OUTPUTS,
  //    Lights::NUM_LIGHTS);
  //
  // Configure each param, input, output, and light:
  //    configParam(Params::some_param, minValue, maxValue, defaultValue,
  //    "name", "unit"); configInput(Inputs::some_input, "input name");
  //    configOutput(Outputs::some_output, "output name");
  //    configLight(Lights::some_light, "light_name");
  //
  //}

  // Save and restore configuration and settings by overriding
  ...
  //
  // This example shows a simple parameter update rate of 1/64th the sample
  // rate. You can use a ::rack::dsp::timer to update relative to clock time,
  // rather than change based on sample rate.
  //
...
struct BlankModuleWidget : ModuleWidget {
  BlankModuleWidget(BlankModule *module) {
    ...
    setPanel(createPanel(asset::plugin(pluginInstance, "res/Blank.svg"),
                         asset::plugin(pluginInstance, "res/Blank-dark.svg")));
...
//     Model *modelSomething = createModel<SomethingModule,
//     SomethingModuleWidget>("slug-something");
